...
### Genetic correlation in base population

```{r}
summary(filter(stats_avoid_cxc_snp, generation == 1)$cor_g)
qplot(filter(stats_avoid_cxc_snp, generation == 1)$cor_g)
```
...
Alternative fig 3 with regression lines:

```{r}
fig3_alt <- (fig3_effect_g1 + geom_smooth(method = lm, se = FALSE, show.legend = FALSE)) + 
  (fig3_effect_g2 + geom_smooth(method = lm, se = FALSE, show.legend = FALSE)) +
  fig3_freq_scatter +
  plot_layout(ncol = 2,
              guides = "collect") +
  plot_annotation(tag_levels = "A") & theme(legend.position = "bottom")
