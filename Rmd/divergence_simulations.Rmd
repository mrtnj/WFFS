...
ggplot() +
  geom_line(aes(x = generation,
                ...
                group = replicate),
            colour = "grey",
            data = carriers_combined_selection) +
  geom_pointrange(aes(x = generation,
                      y = average,
                      ymin = lower,
                      ymax = upper),
                  data = sires_used_summary) +
  facet_wrap(case ~ selection) +
  theme_bw() +
  theme(panel.grid = element_blank())
...
Alternative fig 3 with same x-axis:
...
### Focus in on relevant paramter region

```{r}
filter(balancing_stats_combined,
       lethal_qtl_effect_g1 > 5 &
         lethal_qtl_effect_g1 < 10 &
         lethal_qtl_effect_g2 > -2 &
         lethal_qtl_effect_g2 < 2)
```
